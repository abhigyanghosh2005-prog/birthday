...
export default function CakeScreen({ onNext, onDecorate }) {
 ...
 useEffect(() => {
    // This runs automatically the moment the Cake Screen appears
    if (audioRef.current) {
      audioRef.current.volume = 0.5; // Set volume to 50%
      audioRef.current.play().catch((error) => {
        console.log("Surprise music blocked by browser. It needs one click first!");
      });
    }
  }, []);
  ...
  return (
    <div className="relative">
      {/* The audio element MUST have the 'autoPlay' attribute for the surprise effect */}
      <audio ref={audioRef} src="/birthday-song.mp3" autoPlay loop />

    <div className="px-4 md:px-6 py-10 text-center relative">
      {lit && (
        <motion.div className="fixed top-40 lg:top-40 left-0 w-full text-center text-[40px] md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-fuchsia-400 to-pink-400 drop-shadow leading-tight px-4"
